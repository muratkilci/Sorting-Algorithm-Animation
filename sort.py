...
class Sort:
    ...
    def radix_sort(self):
        ...
        exp = 1
        while max_ // exp > 0:
            opt = {'highlight': [(i, 'b') for i in range(len(self.data))]}
            if self.put_data(**opt) == 'close':
                return 'close'
            buckets = [[] for _ in range(10)]
            for i in range(len(self.data)):
                buckets[self.data[i] // exp % 10].append(self.data[i])
            self.data = []
    ...
    def counting_sort(self):
        ...
        for i in range(len(count)):
            self.data.extend([i + min_] * count[i])
            opt = {'data': self.data.copy()}
            if self.put_data(**opt) == 'close':
                return 'close'
        self.put_data(finish=True)
    ...
    def bucket_sort(self):
        ...
        buckets = [[] for _ in range(10)]
        for i in self.data:
            buckets[int((i - min_) / (max_ - min_) * 10)].append(i)
        self.data = []
        for bucket in buckets:
            bucket.sort()
            self.data.extend(bucket)
